...
export class TextModel
		...
		if (contentChanges.length !== 0) {
			...
			for (let i = 0, len = contentChanges.length; i < len; i++) {
				...
				const decorationsWithInjectedTextInEditedRange =
					this._decorationsTree.getDecoratedTextInInterval(
						this,
				...
				const {
					inlineClassNames,
					lineInjectedTexts: injectedTextInEditedRange,
				} = lineMetaFromDecorations(
				...
				const inlineClassNamesInEditedRangeQueue = new ArrayQueue(
					inlineClassNames,
				);
				...
				for (let j = editingLinesCnt; j >= 0; j--) {
					...
					inlineClassNamesInEditedRangeQueue.takeFromEndWhile(
						(r) => r.lineNumber > currentEditLineNumber,
					);
					const inlineClassNamesInCurrentLine =
						inlineClassNamesInEditedRangeQueue.takeFromEndWhile(
							(r) => r.lineNumber === currentEditLineNumber,
						);
				...
				if (editingLinesCnt < insertingLinesCnt) {
					...
					for (let i = 0; i < cnt; i++) {
						...
						inlineClassNamesInEditedRangeQueue.takeWhile(
							(r) => r.lineNumber < lineNumber,
						);
						allInlineClassNames[i] =
							inlineClassNamesInEditedRangeQueue.takeWhile(
								(r) => r.lineNumber === lineNumber,
							);
					}
	...
	private handleBeforeFireDecorationsChangedEvent(
		...
		if (affectedInjectedTextLines && affectedInjectedTextLines.size > 0) {
			...
			const lineChangeEvents = affectedLines.map((lineNumber) => {
				const { inlineClassNames, lineInjectedTexts } =
					this._getLineWrappingMetaInLine(lineNumber);

				return new ModelRawLineChanged(
					lineNumber,
					this.getLineContent(lineNumber),
					lineInjectedTexts,
					inlineClassNames.filter(
						(element) => element.lineNumber === lineNumber,
					),
				);
			});
	...
	private _getLineWrappingMetaInLine(lineNumber: number) {
		...
		const result = this._decorationsTree.searchTextDecorations(
			this,
			startOffset,
			endOffset,
			0,
				...
				if (oldDecorationIndex < oldDecorationsLen) {
					...
					if (node) {
						...
						if (node.options.inlineClassNameAffectsLetterSpacing) {
							const nodeRange = this._decorationsTree.getNodeRange(this, node);
							for (
								let lineNumber = nodeRange.startLineNumber;
								lineNumber <= nodeRange.endLineNumber;
								lineNumber++
							) {
								this._onDidChangeDecorations.recordLineAffectedByInjectedText(
									lineNumber,
...
class DecorationsTrees {
	...
	public getDecoratedTextInInterval(
		host: IDecorationsTreesHost,
		start: number,
		end: number,
		filterOwnerId: number,
		...
		const result = this._injectedTextDecorationsTree
			.intervalSearch(start, end, filterOwnerId, false, false, versionId, false)
			.concat(
				this._decorationsTree0.intervalSearch(
					start,
					end,
					filterOwnerId,
					false,
	...
	public getAllTextDecorations(
		host: IDecorationsTreesHost,
		filterOwnerId: number,
	): model.IModelDecoration[] {
		const versionId = host.getVersionId();
		const result = this._injectedTextDecorationsTree
			.search(filterOwnerId, false, false, versionId, false)
			.concat(
				this._decorationsTree0.search(
					filterOwnerId,
					true,
					false,
					versionId,
	...
	public searchTextDecorations(
		...
		const versionId = host.getVersionId();
		let result = this._decorationsTree0.intervalSearch(
			start,
			end,
			filterOwnerId,
			true,
		...
		result = result.concat(
			this._injectedTextDecorationsTree.intervalSearch(
				start,
				end,
				filterOwnerId,
				true,
