	...
	private handleBeforeFireDecorationsChangedEvent(affectedInjectedTextLines: Set<number> | null, affectedLineHeights: Set<LineHeightChangingDecoration> | null, affectedFontLines: Set<LineFontChangingDecoration> | null): void {
		...
		if (affectedInjectedTextLines && affectedInjectedTextLines.size > 0) {
			...
			const lineChangeEvents = affectedLines.map(lineNumber => {
				...
				return new ModelRawLineChanged(lineNumber, this.getLineContent(lineNumber), lineInjectedTexts, inlineClassNames.filter((element) => element.lineNumber === lineNumber));
			});
...
class DecorationsTrees {
	...
	public getDecoratedTextInInterval(host: IDecorationsTreesHost, start: number, end: number, filterOwnerId: number): model.IModelDecoration[] {
		const versionId = host.getVersionId();
		let result = this._injectedTextDecorationsTree.intervalSearch(start, end, filterOwnerId, false, false, versionId, false);
		result = result.concat(this._decorationsTree0.intervalSearch(start, end, filterOwnerId, false, false, versionId, false));
		return this._ensureNodesHaveRanges(host, result).filter((i) => i.options.showIfCollapsed || !i.range.isEmpty());
	}
	...
	public getAllTextDecorations(host: IDecorationsTreesHost, filterOwnerId: number): model.IModelDecoration[] {
		const versionId = host.getVersionId();
		let result = this._injectedTextDecorationsTree.search(filterOwnerId, false, false, versionId, false);
		result = result.concat(this._decorationsTree0.search(filterOwnerId, true, false, versionId, false));
		return this._ensureNodesHaveRanges(host, result);
	}
