		...
		return decorations.map((d) => {
			...
			return {
				...
				lineHeight: d.options.lineHeight
					? d.options.lineHeight * defaultLineHeight
					: 0,
			};
		});
	}
				...
				switch (change.changeType) {
					case textModelEvents.RawContentChangedType.LinesInserted: {
						for (let lineIdx = 0; lineIdx < change.detail.length; lineIdx++) {
							...
							if (inlineClassNames) {
								inlineClassNames = inlineClassNames.filter(
									(element) =>
										!element.ownerId || element.ownerId === this._editorId,
								);
							}
					...
					case textModelEvents.RawContentChangedType.LineChanged: {
						...
						let inlineClassNames: textModelEvents.InlineClassName[] | null =
							null;
						if (change.inlineClassNames) {
							inlineClassNames = change.inlineClassNames.filter(
								(element) =>
									!element.ownerId || element.ownerId === this._editorId,
							);
						}
